...
export async function updateTransaction(userId: number, input: UpdateTransactionInput): Promise<Transaction> {
  try {
    ...
    // If category_id is being updated, verify the category exists and belongs to the user
    ...
    if (input.transaction_date !== undefined) {
      updateData['transaction_date'] = new Date(input.transaction_date);
    }
